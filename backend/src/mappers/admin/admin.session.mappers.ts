...
export const toAdminSessionResponseDTO = (session: ISportsSessionDTOWithCategory | IFitnessSessionDTOWithCategory) => {
  ...
  const category = isFitness ? (session as IFitnessSessionDTOWithCategory).fitnessCategory.programName : (session as ISportsSessionDTOWithCategory).sportCategory.sportName;
  return {
...
export const toAdminSessionActionResponseDTO = (session: ISportsSession | IFitnessSession) => {
  const timezone = getTimezone();
  const isFitness = 'fitnessCategory' in session;
  const category = isFitness ? (session as IFitnessSession).fitnessCategory.toString() : (session as ISportsSession).sportCategory.toString();
  return {
    id: session._id.toString(),
    sessionName: session.sessionName,
...
export const toAdminSessionDetailedViewDTO = (session: SportsSessionDetails | FitnessSessionDetails) => {
  const timezone = getTimezone();
  const isFitness = 'fitnessCategory' in session;
  const category = isFitness ? (session as unknown as IFitnessSession).fitnessCategory.toString() : (session as unknown as ISportsSession).sportCategory.toString();
  return {
    id: session._id.toString(),
    trainer: session.trainerId
      ? {
