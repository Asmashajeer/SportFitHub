...
const VenueSchema = z.object({
  ...
  }).optional().default({ type: 'Point', coordinates: [0, 0] }),
});
...
export const fitnessSessionSchema = z
  .object({
    ...
    cancellationPolicy: z
        .string()
        .min(10, "Policy briefing is too short")
        .max(500, "Policy briefing is too long")
        .default("Full refund available if cancelled at least 24 hours before start time."),
    ...
    bookingDeadline: z
        .number()
        .min(0, "Deadline cannot be negative")
        .max(72, "Deadline cannot exceed 3 days")
        .default(2), 
  })
