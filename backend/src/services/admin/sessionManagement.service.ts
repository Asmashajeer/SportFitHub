...
//-------------------get session Stats-----------
  async getSessionStats():Promise<SessionStatsResponseDTO> {
    const [sportsStats, fitnessStats] = await Promise.all([
      this._sportsSessionRepo.getSessionStats(),
      this._fitnessSessionRepo.getSessionStats(),
    ]);
  ...
  async getSessions(  sessionModel: string,  filter: AdminSessionFilterDTO): Promise<AdminSessionsResponseDTO> {
      ...
      const repo = sessionModel === PAYLOAD_MODEL.SPORT_SESSION
        ? this._sportsSessionRepo
        : this._fitnessSessionRepo;
