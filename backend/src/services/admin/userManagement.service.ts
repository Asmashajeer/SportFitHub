...
export class UserManagementService implements IUserManagementService {
  ...
  getUsers = async (filters: getAllUsersRequestDTO): Promise<getAllusersResponseDTO> => {
    ...
    const [usersData, totalCount] = await Promise.all([this._userRepo.findAll(query, { skip, limit }), this._userRepo.countOfUsers(query)]);
  ...
  deleteUser = async (id: string): Promise<usersResposeDTO> => {
    ...
    if (user.roles.includes(UserRole.USER)) {
      const [sessions, wallet] = await Promise.all([this._bookingSessionRepo.find({ userId: user.id, status: BOOKING_SESSION_STATUS.SCHEDULED }), this._walletRepo.findByUserId(user._id)]);
