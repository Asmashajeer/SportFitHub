...
//--------------all ltrainers-----------
  ...
  async getPendingTrainers(user:AuthUser): Promise<PendingTrainersBasicDTO[]> {
    ...
    const pendingTrainersBasicData: PendingTrainersBasicDTO[] =  result.map(trainer =>
  ...
  async updateTrainerStatus(
    ...
    private async _sendDocumentStatusNotification(
      trainer: any,
      targetField: 'certificationInfo' | 'idVerification',
      status: string,
      reason: string
    ...
    private async _sendTrainerStatusNotification(
      trainer: any,
      status: TRAINER_STATUS,
      reason: string
    ) {
