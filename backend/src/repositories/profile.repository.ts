...
export class ProfileRepository extends BaseRepository<IProfile> implements IProfileRepository {
  ...
  // //update profile
  // async updateProfile(id: string | Types.ObjectId, updateData: Partial<IProfile>): Promise<IProfile> {
  //   return await this.model.findOneAndUpdate({id, updateData}, { new: true }).exec();
