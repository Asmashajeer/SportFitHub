...
export class FitnessRepository extends BaseRepository<IFitnessProgram> implements IFitnessRespository {
  constructor(model: Model<IFitnessProgram>) {
    super(model);
  }
  async findAll(query: FilterQuery<IFitnessProgram>, options: { skip: number; limit: number }) {
    return await this.model.find(query).sort({ programName: 1 }).skip(options.skip).limit(options.limit);
  }
}
