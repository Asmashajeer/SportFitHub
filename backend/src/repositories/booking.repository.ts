...
export  class BookingRepository extends BaseRepository<IBooking> implements IBookingRepository{
    ...
    async findBySessionId(stripeSessionId :string):Promise<IBooking|null>{
      return await this.model.findOne({stripeSessionId :stripeSessionId })
        ...
        const bookings = await this.model.aggregate([
          ...
          },
          {
            $addFields: {
              sessionDoc: {
                $ifNull: [
                  { $arrayElemAt: ['$sportSession', 0] },
                  { $arrayElemAt: ['$fitnessSession', 0] }
                ]
              }
            }
          },
          ...(search ? [{
              $match: {
                $or: [
                  { 'sessionDoc.sessionName': { $regex: search, $options: 'i' } },
    ...
    async findByIdwithDetails(id: string|Types.ObjectId){
      ...
      .populate('sessionId', 'sessionName trainerId sessionType');
