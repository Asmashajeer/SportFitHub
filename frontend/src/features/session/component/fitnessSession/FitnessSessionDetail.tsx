...
import { Clock, MapPin, Users, Shield, ArrowLeft, CheckCircle2, Star, Flame, UserCircle, Maximize } from 'lucide-react';
...
import { BOOKING_TYPE, PAYLOAD_MODEL, Review_Type, ROLES, SESSION_MODE } from '@/constants/constants';
...
import type { ISlots, PricePlan, Pricing, TimeSlot } from '../../store/session.types';
...
const FitnessSessionDetail = () => {
  ...
  const [session, setSession] = useState<FitnessSessionDetailedPublicResponseData | null>(null);
  ...
  if (!session) {
    return (
      <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center p-6 text-center">
        ...
        <Button onClick={() => navigate('/fitness')} className="bg-emerald-500 text-gray-400 font-bold">
          Back to Sessions
        </Button>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white font-sans selection:bg-emerald-500/30">
      {/* STICKY NAVIGATION */}
      <nav className="sticky top-16 z-50 bg-transparent backdrop-blur-md border-b border-zinc-900 px-4 sm:px-6 py-4 flex items-center justify-between">
          ...
          <div className="lg:col-span-8">
            <div className="relative h-64 sm:h-80 lg:h-96 rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden shadow-2xl border border-zinc-900">
              ...
              <div className="absolute bottom-4 sm:bottom-8 left-4 sm:left-8 flex gap-2 sm:gap-3 flex-wrap">
                <Badge className="bg-emerald-500 text-black font-black uppercase italic px-3 sm:px-4 py-1 border-none text-[10px] sm:text-xs">{session.fitnessCategory?.programName}</Badge>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-stretch">
          ...
          <div className="lg:col-span-4">
            <div className="lg:sticky lg:top-28 h-full flex flex-col bg-zinc-900 p-5 sm:p-6 rounded-[1.75rem] border border-zinc-800 shadow-2xl relative overflow-hidden">
              ...
              <p className="text-[11px] font-black uppercase text-zinc-500 tracking-[0.2em] mb-4 relative">Select Pricing Plan</p>
            ...
            <div className="bg-zinc-900 p-5 sm:p-8 rounded-[2rem] border border-zinc-800/50 space-y-6">
              ...
              <div className="flex flex-wrap gap-3">
                {session.amenities?.map((item: string, i: number) => (
                  <span key={i} className="flex items-center gap-2 bg-zinc-950 px-3 sm:px-4 py-2 rounded-xl text-xs font-bold text-zinc-200 border border-zinc-800">
...
export default FitnessSessionDetail;
