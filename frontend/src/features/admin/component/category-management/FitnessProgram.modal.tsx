    ...
    try{
      if(Object.values(program).length>0){
          if(isEdit){
            ...
            const {newProgram}=  await CategoryMangementService.addProgram(program);
  ...
  return (
    <Dialog>      
      ...
      <DialogContent className="sm:max-w-106.25">
        <DialogHeader>
          <DialogTitle>{isEdit?'Edit the Program':'Add New fitness program'}</DialogTitle>
        </DialogHeader>
        
        <div className="grid gap-4 py-4">
          <div className="grid gap-2">
            <Label htmlFor="programName">Program Name</Label>
            <Input name="programName" value={program.programName} onChange={(e)=>handleChange(e)} placeholder="e.g. HIIT " />
          </div>
          
          <div className="grid gap-2">
            <Label htmlFor="slug" className="text-muted-foreground">URL Slug (Auto-generated)</Label>
            <Input name="slug" value={program.slug} readOnly className="bg-muted" />
          </div>
         
          <div className="grid gap-2">
            <Label htmlFor="description" className="text-foreground">Description</Label>
            <Textarea name="description" value={program.description} onChange={(e)=>handleChange(e)} className="bg-muted" />
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} type="submit">{isEdit?'Save Changes':'Save program'}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
