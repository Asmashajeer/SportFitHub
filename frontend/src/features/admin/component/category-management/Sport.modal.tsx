  ...
  return (
    <Dialog>      
      ...
      <DialogContent className="sm:max-w-106.25">
        <DialogHeader>
          <DialogTitle>{isEdit?'Edit the Sport':'Add New Sport'}</DialogTitle>
        </DialogHeader>
        
        <div className="grid gap-4 py-4">
          <div className="grid gap-2">
            <Label htmlFor="sportName">Sport Name</Label>
            <Input name="sportName" value={sport.sportName} onChange={(e)=>handleChange(e)} placeholder="e.g.Soccer " />
          </div>
          
          <div className="grid gap-2">
            <Label htmlFor="slug" className="text-muted-foreground">URL Slug (Auto-generated)</Label>
            <Input name="slug" value={sport.slug} readOnly className="bg-muted" />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="icon" className="text-foreground">Icon <span className="text-xs text-muted-foreground">(Optional)</span></Label>
            <Input name="icon" value={sport.icon} onChange={(e)=>handleChange(e)} className="bg-muted" />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="description" className="text-foreground">Description</Label>
            <Textarea name="description" value={sport.description} onChange={(e)=>handleChange(e)} className="bg-muted" />
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} type="submit">{isEdit?'Save changes':'Save Sport'}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
