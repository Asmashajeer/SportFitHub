...
import { DOC_VERIFY_STATUS, TRAINER_STATUS, type Doc_status_type } from '@/constants/constants';
...
export const TrainerApprovalView = ({ trainer, onClose }: { trainer: TrainerOverView; onClose: () => void }) => {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-1 gap-6">
      ...
      <div className="lg:col-span-2 space-y-4">
        <Accordion type="single" collapsible className="w-full">
          ...
          <AccordionItem value="Branding" className="border rounded-lg px-4">
            <AccordionTrigger
              className="text-[13px] hover:no-underline py-4
            ...
            <AccordionContent className="pb-4 border-t pt-4 bg-zinc-800/70 p-4">
              <div className="grid grid-cols-2 gap-y-4">
                <div>
                  <p className="text-xs font-semibold text-muted-foreground uppercase">DisplayName</p>
                  <p className="text-sm font-medium">{selectedTrainer?.displayName}</p>
                </div>
                <div>
                  <p className="text-xs font-semibold text-muted-foreground uppercase">Core Discipline </p>
                ...
                <div className="col-span-2">
                  ...
                  <p className="text-sm font-medium">{selectedTrainer?.specialties.join(', ')}</p>
                </div>
                <div>
                  <p className="text-sm font-semibold text-muted-foreground uppercase">Experience : </p> <span className="text-sm font-medium">{selectedTrainer?.experience}</span>
                </div>
                <div>
                  <hr />
          ...
          <AccordionItem value="personal" className="border rounded-lg px-4">
            ...
            >
              <div className="flex items-center gap-3">
                <ShieldCheck className="text-green-600 h-5 w-5" />
                <span className="font-bold">Personal & Contact Information</span>
              </div>
            </AccordionTrigger>
            <AccordionContent className="pb-4 border-t pt-4  bg-zinc-800/70 p-4">
              <div className="grid grid-cols-2 gap-y-4">
                <div>
                  ...
                  <p className="text-sm font-medium">{selectedTrainer?.personalInfo.fullName}</p>
                </div>
                <div>
                ...
                <div>
                  <p className="text-xs  font-semibold text-muted-foreground uppercase">Gender </p>
                  <p>{selectedTrainer?.personalInfo.gender}</p>
                </div>
                <div>
                  <p className="text-xs font-semibold text-muted-foreground uppercase">Phone </p>
                  <p className="text-sm font-medium">{selectedTrainer?.personalInfo.phone}</p>
                </div>

                <div className="col-span-2">
                  <p className="text-xs font-semibold text-muted-foreground uppercase">Address</p>
                  <p className="text-sm">
                    {selectedTrainer?.personalInfo.address.street}, {selectedTrainer?.personalInfo.address.city}, {selectedTrainer?.personalInfo.address.state} -{' '}
          ...
          <AccordionItem value="Certificates" className="border rounded-lg px-4">
            ...
            <AccordionContent className="pb-4 border-t pt-4  bg-zinc-800/70 p-4">
              ...
              <div className="grid grid-cols-5 gap-y-4">
                {selectedTrainer?.certificationInfo.documents.map((cert: ICertification, i: number) => (
                  <div key={i} className="text-sm p-3 border rounded-md hover:bg-muted/30">
                    <p className="font-normal text-primary">Name: {cert.name}</p>
                    <p className="text-xs text-muted-foreground">Issued: {new Date(cert.issuedAt).toLocaleDateString()}</p>
                    <p className="text-xs text-muted-foreground mb-2">Expires: {new Date(cert.validUpto).toLocaleDateString()}</p>

                    <Button variant="link" className="p-0 h-auto text-xs" asChild>
                      <a onClick={() => getCertificate(cert.id)} target="_blank" rel="noopener noreferrer">
                        View Document
                      </a>
                    </Button>
                  </div>
              ...
              <div className="flex items-center text-start  justify-between">
                <div className="flex items-center   justify-evenly gap-4 px-2">
                  <div className="px-2">
                    ...
                    <Badge variant={selectedTrainer?.idVerification.verified ? 'default' : 'destructive'}>{selectedTrainer?.idVerification.status}</Badge>
                  </div>
                  <div className="px-2">
                  ...
                  <Button variant="outline" size="sm" asChild>
                    <a onClick={getIdAttachment} target="_blank">
          ...
          <AccordionItem value="personal" className="border rounded-lg px-4">
            ...
            <AccordionContent className="pb-4 border-t pt-4  bg-zinc-800/70 p-4">
              <div className="grid grid-cols-2 gap-y-4">
                ...
                <div>
                  <p className="text-xs font-semibold text-muted-foreground uppercase">
                    Availabilty: <span className="text-sm font-medium">{selectedTrainer?.availability.isAvailable ? 'Active' : 'Not Available'} </span>
                  </p>
                  <div className="space-y-2">
                    {selectedTrainer?.availability &&
                      Object.entries(selectedTrainer.availability)
                        .filter(([key, value]) => key !== 'isAvailable' && typeof value === 'object' && value.available)
                        .map(([day, info]: [string, any]) => (
                          <div key={day} className="flex justify-between items-center text-sm border-b pb-1 border-muted/50">
      ...
      <div className="space-y-4">
        ...
        <div className="border rounded-lg bg-secondary/10 p-4 sticky top-4">
                  ...
                  <div className="bg-destructive/5 border border-destructive/20 p-4 rounded-lg space-y-3 animate-in fade-in slide-in-from-top-1">
                    ...
                    <div className="flex justify-end gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (!selectedTrainer?.id) return;
                          trainerApplicationStatus(selectedTrainer, TRAINER_STATUS.REJECTED, rejectionReason);
                          setRejectionTarget(null);
                          setRejectionReason('');
                        }}
                      >
          ...
          <p className="text-[10px] text-center text-muted-foreground mt-4 italic">Approval will notify the selectedTrainer? and activate their public profile.</p>
        </div>
      </div>
    </div>
  );
