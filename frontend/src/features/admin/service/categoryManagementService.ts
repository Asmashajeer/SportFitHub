...
import type { FitnessData, queryParamsOptions, SportData } from "../store/types";
...
export const CategoryMangementService={
    ...
    toggleSportStatus:async(id:string)=>{
          const response=await  api.patch(ADMIN_ROUTES.TOGGLE_SPORT_STATUS+'/'+id);
        return response.data;
    },
    ...
    toggleProgramStatus:async(id:string)=>{
          const response=await  api.patch(ADMIN_ROUTES.TOGGLE_PROGRAM_STATUS+'/'+id);
        return response.data;
    },
