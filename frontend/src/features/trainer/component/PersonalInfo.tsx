...
const PersonalInfoForm:React.FC<PersonalInfoFormProps> = ({ onNext, onBack }) => {
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          {errors?.personalInfo?.fullName && (
            <p className="text-xs text-red-500">{errors.personalInfo.fullName.message}</p>
          )}
        </div>
        <div className="space-y-2">
          ...
          {errors?.personalInfo?.DOB && (
            <p className="text-xs text-red-500">{errors.personalInfo.DOB.message}</p>
          )}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-red-500">{errors.personalInfo.phone.message}</p>
          )}
          </div>
          ...
          <div className="space-y-2">
            ...
            <Select 
              ...
              onValueChange={(val) => handleSelectChange("personalInfo.gender", val)}
            >
              <SelectTrigger>
        ...
        <div className="pt-4 border-t flex items-center gap-2 text-muted-foreground">
          ...
          <span className="text-sm font-semibold uppercase tracking-wider">Address Details</span>
        </div>
        ...
        <div className="space-y-4">
          <h3 className="text-sm font-semibold uppercase text-muted-foreground">Verification</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              {errors?.idVerification?.idNumber && (
                <p className="text-xs text-red-500">{errors.idVerification.idNumber.message}</p>
              )}
            </div>
            {/* ID File Upload */}
            <div className="space-y-2">
            ...
            <div className="flex items-center justify-center w-full">
                ...
                <div className="flex flex-col items-center justify-center pt-5 pb-3">
                    ...
                    <p className={`text-sm ${watch("idVerification.idAttachment")?"text-green-500": "text-gray-500"} `}>
                       {watch("idVerification.idAttachment")? `File Uploaded:${watch("idVerification.idType")} `: "Click to upload ID attachment"}                  
                ...
                <input 
                    type="file" 
                    className="hidden" 
                    {...register("idVerification.idAttachment", { required: "ID attachment is required" })} 
                />
                {errors?.idVerification?.idAttachment&& (
                  <p className="text-xs text-red-500">{errors.idVerification.idAttachment.message}</p>
                )}
                </label>
            </div>
