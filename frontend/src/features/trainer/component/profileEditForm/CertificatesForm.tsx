   ...
   const onSubmit = async ( data: CertificatesFormValues) => {
    ...
    if(profile?.userId){
       ...
       const folderPath = `trainers/${userId}/${profile?.category}`;
          const uploadedCerts=await Promise.all (data.documents.map(async (doc:any)=>{
            const newFile = doc.file?.[0] || (doc.file instanceof File ? doc.file : null);
            if (newFile) {
        ...
        try{     
          if(profile){
          const updatedProfile= await trainerService.updateCertificationInfo(profile?.id,"certificationInfo",  cleanCerts);
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div key={field.id} className="p-4 border rounded-lg relative space-y-4 bg-background/50">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input type="date" {...register(`documents.${index}.issuedAt` as const)} />
            </div>

            <div className="space-y-2">
              <Label>Valid Until</Label>
              <Input type="date" {...register(`documents.${index}.validUpto` as const)} />
            </div>
          </div>
        </div>
      ))}
      ...
      <div className="flex justify-between mt-4">
        ...
        <div className="flex gap-2">
          ...
          <Button type="submit" className="bg-trainer-primary">Save Changes</Button>
        </div>
      </div>
    </form>
  );
