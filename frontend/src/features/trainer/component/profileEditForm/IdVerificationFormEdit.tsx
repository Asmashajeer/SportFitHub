...
import { GOVT_ID_TYPE, UPLOAD_TYPE, type Govt_Id_type } from '@/constants/constants';
...
const IdVerificationFormEdit=({initialData,onCancel}:props) =>{
    ...
    const{register,formState:{errors},handleSubmit,watch,control}= form;
    ...
    const onSubmit=async(data:idVerification)=>{
        if(profile){
            ...
            try{
                ...
                const url=await uploadService.upload(data.idAttachment?.[0],folderPath,userId,UPLOAD_TYPE.ID_ATTACHMENT);
                if(url){
                const idverifcationInfo={
                    ...
                    const updatedProfile=await trainerService.updateIdverification(profile?.id,idverifcationInfo);
  ...
  return (
    <Form {...form}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-2 border-2 p-4 my-4" >
            ...
            <div className="space-y-2">
                ...
                <input 
                    type="file" 
                    className="" 
                    {...register("idAttachment", { required: "ID attachment is required" })} 
                />
                {errors?.idAttachment&& (
                <p className="text-xs text-red-500">{errors.idAttachment.message}</p>
                )}
            </div>
