...
interface props{
    initialData:{     
        ...
        availability:{
            isAvailable: boolean;
             Monday: dayAvailability; Tuesday:dayAvailability; Wednesday:dayAvailability; Thursday:dayAvailability; Friday:dayAvailability; Saturday:dayAvailability; Sunday:dayAvailability; 
        }  ,  
    }   
      onCancel:()=>void
}
...
const AvailabilityFormEdit = ({ initialData,  onCancel }:props) => {
  ...
  const onSubmit = async (data:AvailabiltyPricing) => {
    if(profile){
        const updatedProfile=await trainerService.updateAvailability_Pricing(profile?.id, data);
